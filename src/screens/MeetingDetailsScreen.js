  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        />
        {loading ? (
          <Text style={styles.meta}>Loading meeting details...</Text>
        ) : null}
        ...
        <CostCard
          label={'ESTIMATED PEOPLE COST'}
          duration={currentMeeting.duration || 'duration not available'}
          value={currentMeeting.cost}
        />
        <Text style={styles.meta}>
          This meeting is created by{' '}
          {currentMeeting.creator?.name || 'Unknown user'}
        </Text>
        ...
        {attendees.map((name, index) => (
          <View key={name} style={styles.attendee}>
            <ProfileAvatar
              backgroundColor={
                index % 2 ? undefined : styles.avatarCool.backgroundColor
              }
              name={name}
              size={40}
